...
from project import app, crypt, db, cart_utils, Users, Carts, Products, Customer_Cart, Billing, cart_utils
import secrets
import random
import string
import os
...
def dashboard():
    ...
    print(f"Logged In : {logged_in}")
    if logged_in:
        if logged_in_detail['admin'] == 'True':
            return render_template('dashboard.html', title='Admin Dashboard', logged_in=logged_in, user=logged_in_detail, usrcrsr=User_cursor, crtcrsr=Cart_cursor, prdcrsr=Product_cursor, cccrsr=Customer_cart_cursor, bilcrsr=Billing_cursor, bchart=billing_chart_data)#, graphJSON=graphJSON, ids=ids)
        else:
            return render_template('userdashboard.html', title='User Dashboard', logged_in=logged_in, user=logged_in_detail, usrcrsr=Users, crtcrsr=Carts, prdcrsr=Products, cccrsr=Customer_Cart, bilcrsr=Billing)#, graphJSON=graphJSON, ids=ids)
    else:
        return render_template('dashboard.html', title='Dashboard', logged_in=logged_in,)#, graphJSON=graphJSON, ids=ids)
...
@app.route('/pi_device',methods=['GET','POST'])
def use_pi():
    if request.method == 'POST':
        cart_action = request.form.get('cart')
        scan_no = request.form.get('scan')
        if cart_action:
            if cart_action == 'inactive':
                cart_utils.activate(state=False)
            else:
                cart_utils.activate(state=True)
        elif scan_no:
            cart_utils.add_product(scan=int(scan_no))
...
@app.route('/activate_cart/<int:cid>', methods=['POST'])
def activate_cart(cid):
    x=Carts.find_one({'_id':int(cid)})
    newvalue={}
    oldvalue={'_id':cid}
    status=request.form.get('status')
